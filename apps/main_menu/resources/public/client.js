...
function getMainMenuCommands() {
    callServerMethod('main_menu', 'getMainMenuCommands', {})
        .then(result => {
            ...
            MainMenu.Draw();
            // Вызываем событие resize, чтобы другие компоненты могли пересчитать свои размеры
...
const MainMenu = {
    container: null,
    buttons: [], // Будет заполнено через getMainMenuCommands
    
    buttonObjects: [],
    Draw: function () {
        // Используем размеры, совместимые с заголовком формы из UI_classes
        // В UI_classes заголовочные кнопки имеют height '18px' и padding '2px 4px'
        ...
        // Устанавливаем глобальный отступ для форм, если класс Form доступен
        if (typeof Form !== 'undefined') {
            Form.topOffset = menuHeight + borderBottomWidth;
        }

        // Кнопки на всю высоту меню
        const btnHeight = menuHeight;
        const btnWidth = 120; // ширина оставляем гибкой для текста
        ...
        // Толщина меню = высота кнопки
        this.container.style.height = menuHeight + 'px';

        // Цвет меню такой же, как у кнопки (если доступен UIObject)
        ...
        // "Объемный" стиль только снизу (как в классе Button)
        ...
        // Кнопки располагаются вплотную друг к другу и к левому краю
...
<rect x="10" y="10" width="4" height="4"/>
                ...
                // Располагаем кнопку вплотную к предыдущей и к левому краю
                btnElement.style.margin = '0';
                btnElement.style.fontSize = '12px';
                // Подгоняем размер под высоту заголовка
                btnElement.style.height = btnHeight + 'px';
                // btnElement.style.lineHeight = btnHeight + 'px'; // Убираем line-height

                // Используем Flexbox для точного центрирования текста
                ...
                // "Плоский" стиль для меню (если не main)
                if (!isMain) {
                    const setFlat = () => {
                    ...
                    setFlat();
                    // Повторяем после загрузки конфига, так как Button может перерисовать границы
                    ...
                    btnElement.onmouseleave = () => {
                        // Если меню открыто, можно оставить нажатым, но пока просто сбрасываем
                        setFlat();
                    };
                }
            }
...
async function collectMainMenuCommands() {
    // Получаем список приложений из apps.json
    let appsList = [];
    try {
